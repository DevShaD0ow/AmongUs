...
bool URewindSubsystem::GetRewindStatesForTime(float Time, TMap<URewindableComponent*, FTransform>& Out)
{
    for (auto& Elem : ComponentHistories)
    {
        ...
        const auto& A = History.History[Prev];
        const auto& B = History.History[Next];
...
bool URewindSubsystem::VerifyHit(float Timestamp, const FVector& Start, const FRotator& Rot, APlayerState* Target)
{
    ...
    if (!GetRewindStatesForTime(Timestamp, Rewinded))
        return false;
